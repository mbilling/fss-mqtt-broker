...
| Queued/offline messages per session | ✅ count, default 100 000, overflow `drop-oldest` or `reject-newest` (`MQTTD_MAX_QUEUED_MESSAGES`, `MQTTD_QUEUE_OVERFLOW`) — both overflow policies **ack-and-drop** at the cap: the default `drop-oldest` truncates the oldest *already-acked* entries out of the durable queue, `reject-newest` acks and sheds the newest (counted `publish_dropped{reason="queue-overflow"}`); ✖ **no byte-based cap** — accepted, tracked (ADR 0041 amendment T6; issue #241 byte-bounded the in-memory backlog and deliberately did **not** claim this one, because an exact byte total on disk needs a *persisted* per-session counter). The same ack-and-drop arm is now also reachable at the in-memory backlog's byte bound (`publish_dropped{reason="backlog-overflow"}`), which an operator can set low enough to make routine | ✅ count (`max_queued_messages`, default 1000) **and bytes** (`max_queued_bytes`) | ✅ `max_mqueue_len` (default 1000); byte variant n/v | ⚠️ `msq_len`; byte variant n/v | ✅ `max_online_messages` / `max_offline_messages` (1000/1000) |
...
| Disk bound / full-disk behavior | ⚠️ one aggregate high-water mark (`MQTTD_STORE_MAX_BYTES`) → **brownout**: growth writes refused — subscriber acks/reads/expiry continue, while a `QoS` ≥ 1 publisher is **refused** (v5 `0x97`, v3.1.1 no ack + close) rather than acked for a message the store will not take — cross-node too, as a peer-bus verdict; mid-rolling-upgrade an older link degrades to a withheld ack + close (issue #238); disk-full itself fails closed, crash-tested mid-write (ADR 0044 P2). Detection lag ≤ `MQTTD_WATERMARK_POLL` (shipped `v0.9.1`; default 10 s, 1 s within 10% of the mark), so the total can overshoot by one interval's growth. ✖ no per-store quota (ADR 0041 amendment T9) — the aggregate is the enforcement point **because** two of the four stores have no refusable client write (`replicas` grows from peers' committed appends, `lease` from consensus); a WARN naming any store above 70% of the mark is the visibility half | ⚠️ persistence file + autosave, no quota | n/v | n/v | n/v (node-local LevelDB) |
| Total-memory limit | ⚠️ **watermark, not a ceiling**: `MQTTD_MEMORY_MAX_BYTES` → brownout (growth refused; subscriber acks/reads/expiry/resumes continue, while a `QoS` ≥ 1 publisher is refused — v5 `0x97`, v3.1.1 no ack + close, cross-node as a peer-bus verdict (older links mid-upgrade: withheld ack + close) — ADR 0041 T8/T11/T12). Cannot stop RSS rising: the mark is sampled, so overshoot ≤ `MQTTD_WATERMARK_POLL x allocation rate` (shipped `v0.9.1`; default 10 s, 1 s within 10% of the mark) — the container limit is still the hard bound, and it needs `/proc` (Linux). **A genuine loss against mosquitto**: it has a hard cap and we have none, by decision — though the cap is on mosquitto's own tracked allocations rather than on RSS, so it too leaves the container limit as the true bound; it pays for that with failing allocations (refused packets, lost messages), where brownout keeps standing state and tells the publisher | ✅ `memory_limit` (hard cap on its own TRACKED allocations — enforced in mosquitto's allocator wrappers, so it does not bound RSS: library allocations, stacks and fragmentation sit outside it; allocation failures are the cost) | ⚠️ per-connection `force_shutdown` (heap default 32 MiB + mailbox 1000) — kills the connection, not a broker-wide cap | n/v | n/v |
| Per-connection write buffering | ✅ (all four knob names shipped `v0.9.1`) all **three** per-subscriber in-memory structures are operator-lowerable (issue #241, ADR 0041 amendment T10): the flow-control backlog in **messages and bytes** with exact accounting (`MQTTD_MAX_BACKLOG_MESSAGES` default 10 000, `MQTTD_MAX_BACKLOG_BYTES` off by default, drop-oldest, `publish_dropped{reason="backlog-overflow"}`, the WARN naming which bound fired); the **in-flight window** via a ceiling on the effective outbound Receive Maximum (`MQTTD_MAX_INFLIGHT_MESSAGES` — a gate on the wire window, which sheds nothing itself, though the surplus it holds back waits in the drop-oldest backlog, so it is not loss-free end to end; unset it is 65 535, which was the *largest* of the three and previously had no knob at all); and the outbound channel's bytes (`MQTTD_MAX_OUTBOUND_BYTES`, `QoS` 0 shed only) beside its fixed 10 000-packet cap. Unset, the worst case is `(65 535 + 10 000 + 10 000) x MQTTD_MAX_PACKET_SIZE` ≈ 84 GiB per stalled subscriber — the number this row used to state as ~10 GiB counted the backlog alone. ⚠️ the backlog byte bound **sheds already-acked messages without telling the publisher** (the arm below, reached earlier); refusing the publisher instead is accepted as strictly better and tracked (0041-T15). Note the axis split against mosquitto: its `max_queued_bytes` bounds the **offline (disk)** queue, which is our still-open T6 — this is parity on the RAM axis, not on disk; its `max_inflight_messages` is the analogue of our new ceiling | ⚠️ bounded by `max_queued_bytes` (which is the OFFLINE queue) + `max_inflight_messages` | ⚠️ per-connection `force_shutdown` heap cap | n/v | n/v |
...
- 2026-08-18 — Version refresh for the `v0.9.1` release and the 1.0 freeze: header
  ...
  "unreleased" markers are dropped.
- 2026-08-15 — Per-connection write-buffering row rewritten for issue #241 (ADR 0041 T10): all
  three per-subscriber in-memory structures are now operator-lowerable, and the row's old
  "~10 GiB per stalled subscriber" understated the worst case — it counted the flow-control
  backlog alone, while the in-flight window (unset: 65 535 messages, previously with no knob at
  all) is the largest of the three, so the honest unset figure is ~84 GiB. All four knob names
  post-date `v0.9.0` and are marked unreleased accordingly. Corrected on review: the in-flight
  ceiling was described as "a pure gate, nothing dropped", which is true of the gate itself but
