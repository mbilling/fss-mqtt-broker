...
| Will delay | ✅ single node (`min(will delay, session expiry)`, deleted by any new connection for the client id — resume or Clean Start, [MQTT-3.1.2-8] — persisted across restart, fired at or after its deadline and exactly once); ⚠️ in a cluster a resume that lands on **another** node does not cancel a pending will, so a relocated client's will still fires one delay later — bounded, counted, documented | ✅ | ✅ | n/v (documented for bridges only) | ⚠️ lost across netsplit "window of uncertainty" (documented) |
