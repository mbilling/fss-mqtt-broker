...
tasks:
  ...
  - id: 0071-T3
    ...
    status: done
    date: 2026-08-23
    evidence: "Appends route through a dedicated persistent-log-writer OS thread: drain-then-commit into ONE Durability::Immediate transaction (offsets assigned inside the batch's txn, 1-based, monotonic per key, arrival order), whole-batch reject on a failed commit, WriterStats (batches/ops/max_batch) for tests and future metrics. The writer's database handle is released DETERMINISTICALLY: WriterHandle drops the sender then JOINS the thread on the last log clone's drop, so a reopen never races DatabaseAlreadyOpen (ADR 0018 phase 5 preserved — proven by dropping_the_log_releases_the_writer_before_a_reopen, 5 open/append/drop cycles). MEASURED: a_concurrent_append_burst_group_commits_without_changing_the_contract — 8 concurrent writers x 25 appends = 200 ops in 25 fsync'd batches, mean AND max batch 8: every writer shares every barrier, an 8x barrier reduction at that concurrency, with per-key offsets dense/ordered and all records surviving reopen (the contract unchanged). Full mqtt-storage suite (79) green."
---
...
| 0071-T3 | ✅ done | 2026-08-23 | "Appends route through a dedicated persistent-log-writer OS thread: drain-then-commit into ONE Durability::Immediate transaction (offsets assigned inside the batch's txn, 1-based, monotonic per key, arrival order), whole-batch reject on a failed commit, WriterStats (batches/ops/max_batch) for tests and future metrics. The writer's database handle is released DETERMINISTICALLY: WriterHandle drops the sender then JOINS the thread on the last log clone's drop, so a reopen never races DatabaseAlreadyOpen (ADR 0018 phase 5 preserved — proven by dropping_the_log_releases_the_writer_before_a_reopen, 5 open/append/drop cycles). MEASURED: a_concurrent_append_burst_group_commits_without_changing_the_contract — 8 concurrent writers x 25 appends = 200 ops in 25 fsync'd batches, mean AND max batch 8: every writer shares every barrier, an 8x barrier reduction at that concurrency, with per-key offsets dense/ordered and all records surviving reopen (the contract unchanged). Full mqtt-storage suite (79) green." |
<!-- /status-table:0071 -->
