...
tasks:
  ...
  - id: 0073-T4.1
    ...
    date: 2026-09-07
    issue: 565
    notes: "cargo land, €0, CI hours. The #390 grow hand-off race was found exactly this way (deterministically red under full-suite load); this stage exists so the next one like it costs CI minutes, not cluster money. DETERMINISTIC HALF LANDED 2026-09-07: cluster_stress::a_fleet_scale_grow_and_shrink_soak_loses_no_acked_fact grows 3->6->9, shrinks 9->6 by graceful decommission drain, publishes acked QoS 1 facts at every size for 8 durable sessions, and requires every one back from whatever node owns the session afterwards (32 obligations, ~50s). Two guards stop it passing vacuously: at least one session must have CHANGED owner, and at least one must finally be owned by a JOINER — ADR 0073's actual claim, and it means a session served by a node that never votes, since the voter set caps at 5 while the fleet is 9. Oracle liveness was checked by injecting a phantom obligation (it fails, naming what it did get). En route it found that wait_cluster_ready CANNOT be used above the voter cap at all — it waits for voter_count == node count, which never holds past 5 — so the fleet path needed its own readiness helper, and anything else growing past 5 nodes in this harness hits the same wall. NOT DONE, hence in-progress: the SEEDED FAULT SCHEDULE still cannot reach fleet scale, because join_step is bounded to two joins per schedule, so grow/shrink under the fault vocabulary (kills, flaps, disk faults, brownout) is still exercised only at 3-5 nodes; raising that bound changes every existing seed's behaviour and deserves its own decision rather than being slipped in here. OBSERVATION for T4.4, not yet a finding: only 2 of 8 sessions changed owner across 3->6->9->6 and 2 of 8 ended on a joiner — HRW is deliberately minimal-disruption, but the capped-vs-uncapped A/B has less signal the less ownership actually moves, so measure the spread deliberately before paying for the slope run."
  - id: 0073-T4.2
...
| 0073-T4.0 | ⬜ planned | [#564](https://github.com/mbilling/fss-mqtt-broker/issues/564) | — | "Motivation: ADR 0077-T4 measured ~40% variance between freshly provisioned clusters running the SAME binary — wider than the slope T4 measures. Two paid runs would be two cluster draws; the only honest A/B this rig can pay for is both arms on one provisioning, plus a control rung common to both arms and repeated rungs within a provisioning." |
| 0073-T4.1 | 🚧 in-progress | [#565](https://github.com/mbilling/fss-mqtt-broker/issues/565) | 2026-09-07 | "cargo land, €0, CI hours. The #390 grow hand-off race was found exactly this way (deterministically red under full-suite load); this stage exists so the next one like it costs CI minutes, not cluster money. DETERMINISTIC HALF LANDED 2026-09-07: cluster_stress::a_fleet_scale_grow_and_shrink_soak_loses_no_acked_fact grows 3->6->9, shrinks 9->6 by graceful decommission drain, publishes acked QoS 1 facts at every size for 8 durable sessions, and requires every one back from whatever node owns the session afterwards (32 obligations, ~50s). Two guards stop it passing vacuously: at least one session must have CHANGED owner, and at least one must finally be owned by a JOINER — ADR 0073's actual claim, and it means a session served by a node that never votes, since the voter set caps at 5 while the fleet is 9. Oracle liveness was checked by injecting a phantom obligation (it fails, naming what it did get). En route it found that wait_cluster_ready CANNOT be used above the voter cap at all — it waits for voter_count == node count, which never holds past 5 — so the fleet path needed its own readiness helper, and anything else growing past 5 nodes in this harness hits the same wall. NOT DONE, hence in-progress: the SEEDED FAULT SCHEDULE still cannot reach fleet scale, because join_step is bounded to two joins per schedule, so grow/shrink under the fault vocabulary (kills, flaps, disk faults, brownout) is still exercised only at 3-5 nodes; raising that bound changes every existing seed's behaviour and deserves its own decision rather than being slipped in here. OBSERVATION for T4.4, not yet a finding: only 2 of 8 sessions changed owner across 3->6->9->6 and 2 of 8 ended on a joiner — HRW is deliberately minimal-disruption, but the capped-vs-uncapped A/B has less signal the less ownership actually moves, so measure the spread deliberately before paying for the slope run." |
| 0073-T4.2 | ⬜ planned | [#566](https://github.com/mbilling/fss-mqtt-broker/issues/566) | — | "A rig bug found here costs cents. Same pipeline as the real runs, every knob shrunk." |
| 0073-T4.3 | ⬜ planned | [#567](https://github.com/mbilling/fss-mqtt-broker/issues/567) | — | "€2–4, ~30–60 min each. T3's in-tree soak re-run at cloud scale; the last stage where defects are cheap." |
| 0073-T4.4 | ⬜ planned | [#568](https://github.com/mbilling/fss-mqtt-broker/issues/568) | — | "€5–10 in one session. Corrects the original T4 note: the 'needs ~100 dedicated vCPU quota raise' applied to full-ladder shapes; the durable lane's shape fits the existing quota. Issues found here cost an hour and a few euros — the point of the ordering." |
| 0073-T4.5 | ⬜ planned | [#569](https://github.com/mbilling/fss-mqtt-broker/issues/569) | — | "~€30–40. By the time these run, steady-state behaviour — not correctness — should be all they can measure. Publication never cites gate runs: only T4.4's full-profile numbers go in the doc." |
| 0073-T5 | ⬜ planned | [#576](https://github.com/mbilling/fss-mqtt-broker/issues/576) | — | "Every option under #537 Phase 3 optimizes the single mesh; none asks whether one mesh is the right unit at fleet scale. Mesh-wide costs grow with fleet size regardless of those optimizations — per-publish interest fan-out, full interest/shared snapshots gossiped to every peer on every subscription change, and one shared store behind every hub shard. Decide what crosses a cell boundary and which guarantees explicitly do not, against the operational cost of N control planes. Composes with this ADR rather than superseding it; builds on ADRs 0025/0059/0060. Raised by the 2026-09-06 scaling review, final paragraph. |
<!-- /status-table:0073 -->
