...
tasks:
  ...
  - id: 0073-T3
    ...
    status: done
    date: 2026-08-23
    evidence: "The MECHANISM half, in-process (durable_sessions::growing_the_scaled_out_cluster_migrates_ownership_with_zero_acked_loss, ~4s): 3 nodes at voter cap 3 with the scale-out flag on, 120 sessions durably enqueued via their owners; the cluster GROWS to 6 (three learner newcomers); ownership spreads onto the newcomers — asserted to include a LEARNER owner, which ADR 0049 alone could never produce — via the unchanged ADR 0043 P2 eager migration; every pre-grow acked message reads back intact from its post-grow owner. Zero acked loss. The fleet-size version (10 nodes, real load, paid hardware) is folded into T4's measurement run — same falsifier, same rig."
  - id: 0073-T4
...
| 0073-T3 | ✅ done | 2026-08-23 | "The MECHANISM half, in-process (durable_sessions::growing_the_scaled_out_cluster_migrates_ownership_with_zero_acked_loss, ~4s): 3 nodes at voter cap 3 with the scale-out flag on, 120 sessions durably enqueued via their owners; the cluster GROWS to 6 (three learner newcomers); ownership spreads onto the newcomers — asserted to include a LEARNER owner, which ADR 0049 alone could never produce — via the unchanged ADR 0043 P2 eager migration; every pre-grow acked message reads back intact from its post-grow owner. Zero acked loss. The fleet-size version (10 nodes, real load, paid hardware) is folded into T4's measurement run — same falsifier, same rig." |
