...
tasks:
  ...
  - id: 0022-T8
    title: Hot rotation of the gossip signing identity (peer-bus leaf/key file-watched, signer swappable on reload)
    status: done
    date: 2026-08-14
    evidence: "Issue #269, closing the gap the #262 docs correction recorded: the peer-bus CA/cert/key were missing from watched_policy_paths (the watcher sat idle on a full bus) AND the gossip signer was a startup snapshot (NodeGossipSigner: one construction site, no reload path) — so a rotated leaf kept the OLD signing identity until the old cert's notAfter, with no correlating change to point at. Fix inside the existing ADR 0032/0033 architecture: the three paths joined the watch scope, and the signing identity moved behind swim_auth::SignerSlot — signer + cert fingerprint swapped together, read once per sealed datagram (a concurrent rotation can never pair a new key's signature with the old cert), full-cert send counter reset on swap so the first post-rotation datagram primes peers with the complete new leaf. Reloader::attach_gossip_signer rebuilds the signer from the re-read leaf/key in the same atomic validate-before-swap reload as the peer TLS contexts; a bad file rejects the WHOLE reload. Mid-rotation coexistence is per-datagram CA verification (nothing pins one leaf), so a fleet rotates one node at a time. Scope honestly bounded: rotating the CA itself is still restart-bound (verifier + CRL check pin the startup CA) — stated in OPERATIONS.md/values.yaml/ADR 0047 amendment. Tests (each mutation-proven): swim_cluster a_rotated_signing_identity_is_embedded_in_live_gossip_without_a_restart (rotate under a running UDP mesh; the attested-domain observable flips with the swapped cert); mqttd reload_peer_signer a_leaf_rotated_on_disk_is_signing_gossip_within_one_watch_tick (real rcgen PKI on disk, one ConfigWatcher tick, receiver captures the NEW leaf DER; garbage key rejected keeping the running signer, retried until settled); reload a_reload_swaps_the_gossip_signer_and_a_bad_build_rejects_everything; swim_auth a_swapped_signer_embeds_and_signs_with_the_new_certificate_immediately; main the_peer_bus_tls_material_is_file_watched. Mutations: dropping the three watched paths, skipping slot.swap, dropping the sends reset, and dropping the bad-build reject each fail the named test."
---
...
| 0022-T8 | ✅ done | 2026-08-14 | "Issue #269, closing the gap the #262 docs correction recorded: the peer-bus CA/cert/key were missing from watched_policy_paths (the watcher sat idle on a full bus) AND the gossip signer was a startup snapshot (NodeGossipSigner: one construction site, no reload path) — so a rotated leaf kept the OLD signing identity until the old cert's notAfter, with no correlating change to point at. Fix inside the existing ADR 0032/0033 architecture: the three paths joined the watch scope, and the signing identity moved behind swim_auth::SignerSlot — signer + cert fingerprint swapped together, read once per sealed datagram (a concurrent rotation can never pair a new key's signature with the old cert), full-cert send counter reset on swap so the first post-rotation datagram primes peers with the complete new leaf. Reloader::attach_gossip_signer rebuilds the signer from the re-read leaf/key in the same atomic validate-before-swap reload as the peer TLS contexts; a bad file rejects the WHOLE reload. Mid-rotation coexistence is per-datagram CA verification (nothing pins one leaf), so a fleet rotates one node at a time. Scope honestly bounded: rotating the CA itself is still restart-bound (verifier + CRL check pin the startup CA) — stated in OPERATIONS.md/values.yaml/ADR 0047 amendment. Tests (each mutation-proven): swim_cluster a_rotated_signing_identity_is_embedded_in_live_gossip_without_a_restart (rotate under a running UDP mesh; the attested-domain observable flips with the swapped cert); mqttd reload_peer_signer a_leaf_rotated_on_disk_is_signing_gossip_within_one_watch_tick (real rcgen PKI on disk, one ConfigWatcher tick, receiver captures the NEW leaf DER; garbage key rejected keeping the running signer, retried until settled); reload a_reload_swaps_the_gossip_signer_and_a_bad_build_rejects_everything; swim_auth a_swapped_signer_embeds_and_signs_with_the_new_certificate_immediately; main the_peer_bus_tls_material_is_file_watched. Mutations: dropping the three watched paths, skipping slot.swap, dropping the sends reset, and dropping the bad-build reject each fail the named test." |
<!-- /status-table:0022 -->

## Changelog

- **2026-08-14** — T8 (hot rotation of the signing identity, issue #269) landed. The
  signer was the one piece of the gossip security plane that never reloaded: a startup
  snapshot, so a rotated peer-bus leaf kept the old signing identity until the old cert's
  `notAfter`. The identity now lives in a `SignerSlot` read per sealed datagram —
  signer + fingerprint swapped together, send counter reset so the first post-rotation
  datagram carries the full new leaf — rebuilt by the same validate-before-swap reload as
  the peer TLS contexts, and the peer-bus CA/cert/key joined the file-watch scope. Proven
  by a rotate-under-a-running-mesh test and an on-disk one-watch-tick end-to-end test;
  CA rotation itself stays restart-bound (stated in OPERATIONS/values.yaml/ADR 0047).
