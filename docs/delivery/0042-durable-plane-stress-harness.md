...
| 2 | **Adopted-orphan gap loss** (real defect, `acked-durability`): a takeover merge adopts a single-replica orphan into the committed run; the new owner builds on it without re-replicating; a later takeover whose read quorum misses the orphan-holder gaps at that offset and the merge contiguity rule discards the **acked tail above it** — acked messages permanently lost. Reproduce: `durable_sim`, any of hundreds of seeds (e.g. `REPRO_SEED = Some(0)` in the unwaived test) | Found by the T2 simulation's first sweep (2026-07-10) | open — **T6 fixes** (waived in the sweeps, red under `--ignored`) |
