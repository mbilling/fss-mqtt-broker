...
tasks:
  ...
  - id: 0044-P3
    ...
    status: done
    date: 2026-07-17
    evidence: "cluster_upgrade::a_rolling_upgrade_and_rollback_lose_no_acked_fact — TWO binaries for real: the BASELINE is a PINNED full SHA (the 0044-P2 merge; deliberately bumped, never floated, so an incompatible wire/schema reshape FAILS this test until the baseline moves consciously with it — pre-1.0's honest substitute for released-version skew, and the exact machinery 0039-T3 points at two release tags at 1.0), built from a git worktree of that ref into a per-ref cached target dir (its own sources, lockfile, and CARGO_TARGET_DIR; repeat runs pay nothing) or supplied prebuilt via MQTTD_BASELINE_BIN (the nightly tier's path). The ROLL is the operator's motion end to end: ProcNode::terminate() sends SIGTERM — the ADR 0019 graceful stop (drain, flush, SWIM leave), with a bounded wait and SIGKILL escalation exactly as an init system would — then the binary swaps and the node restarts over its SAME data dir and rejoins; each of the SIX rolls (3 up to HEAD, 3 back down to baseline) therefore exercises the full leave→placement-eviction→eager-migration→rejoin→catch-up cycle (the ADR 0043 replace motion) PLUS the cross-version dir reopen in BOTH directions — the ADR 0038 schema gates firing on real files written by the other binary. Acked QoS 1 publishes flow before and after every swap — baseline-only, every mixed-binary window, HEAD-only, and back — and every ack anywhere in the story is a hard obligation; the oracle (resume honesty + full replay) runs after both rolls. Green first run (210s including the cold baseline build; 12 publishes, 7 owed across the windows), green again cached (108s). #[ignore] in the per-PR profile (building a second binary costs minutes); the nightly tier (P4) runs --ignored. Harness: ProcNode gains a per-node binary path; the shared harness moved to tests/proc_common/ (cluster_proc and cluster_upgrade both ride it). Workspace green, clippy zero warnings. The ADR 0043 recorded gap (whole-cluster two-binary rolling upgrade) is CLOSED."
  - id: 0044-P4
...
| 0044-P3 | ✅ done | 2026-07-17 | "cluster_upgrade::a_rolling_upgrade_and_rollback_lose_no_acked_fact — TWO binaries for real: the BASELINE is a PINNED full SHA (the 0044-P2 merge; deliberately bumped, never floated, so an incompatible wire/schema reshape FAILS this test until the baseline moves consciously with it — pre-1.0's honest substitute for released-version skew, and the exact machinery 0039-T3 points at two release tags at 1.0), built from a git worktree of that ref into a per-ref cached target dir (its own sources, lockfile, and CARGO_TARGET_DIR; repeat runs pay nothing) or supplied prebuilt via MQTTD_BASELINE_BIN (the nightly tier's path). The ROLL is the operator's motion end to end: ProcNode::terminate() sends SIGTERM — the ADR 0019 graceful stop (drain, flush, SWIM leave), with a bounded wait and SIGKILL escalation exactly as an init system would — then the binary swaps and the node restarts over its SAME data dir and rejoins; each of the SIX rolls (3 up to HEAD, 3 back down to baseline) therefore exercises the full leave→placement-eviction→eager-migration→rejoin→catch-up cycle (the ADR 0043 replace motion) PLUS the cross-version dir reopen in BOTH directions — the ADR 0038 schema gates firing on real files written by the other binary. Acked QoS 1 publishes flow before and after every swap — baseline-only, every mixed-binary window, HEAD-only, and back — and every ack anywhere in the story is a hard obligation; the oracle (resume honesty + full replay) runs after both rolls. Green first run (210s including the cold baseline build; 12 publishes, 7 owed across the windows), green again cached (108s). #[ignore] in the per-PR profile (building a second binary costs minutes); the nightly tier (P4) runs --ignored. Harness: ProcNode gains a per-node binary path; the shared harness moved to tests/proc_common/ (cluster_proc and cluster_upgrade both ride it). Workspace green, clippy zero warnings. The ADR 0043 recorded gap (whole-cluster two-binary rolling upgrade) is CLOSED." |
| 0044-P4 | ⬜ planned | — |  |
| 0044-P5 | ⬜ planned | — |  |
| 0044-P6 | ⬜ planned | — |  |
| 0044-P7 | ⬜ planned | — |  |
...
- **2026-07-17** — **0044-P3 done: the two-binary rolling upgrade.** A real
  3-node cluster of BASELINE-binary nodes (a pinned SHA built from a git
  worktree into a cached target dir; `MQTTD_BASELINE_BIN` overrides for the
  nightly tier) rolls to HEAD one node at a time and back down again, by the
  operator's motion — SIGTERM graceful stop, binary swap, restart over the same
  data dir, `/readyz` re-admission — with acked load flowing through every
  mixed-binary window and the dir reopened across versions in both directions
  (the ADR 0038 gates firing on real files). Each roll is the full
  leave/eager-migration/rejoin/catch-up cycle. The pinned baseline doubles as a
  compatibility tripwire: an incompatible reshape fails this test until the
  baseline is bumped consciously with it. **The ADR 0043 recorded gap closes**;
  0039-T3's 1.0 skew test is now this test pointed at two release tags.
  `#[ignore]` per-PR (a second binary costs minutes); nightly runs `--ignored`.
