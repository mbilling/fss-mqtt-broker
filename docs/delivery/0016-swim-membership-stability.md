...
tasks:
  ...
  - id: 0016-T6
    ...
    status: done
    date: 2026-07-02
    evidence: "Option (1) of the recorded analysis (CA-attested labels), landed together with 0022-T7 (shared PKI surface). A cluster-bus leaf carrying SAN URI:urn:fss:failure-domain:<label> has its domain attested by the CA: signed_gossip::verify extracts it (a present-but-empty label is rejected as malformed, not ignored) and swim_driver enforces it beside the CN<->from binding — a self-claim (header or self-update) contradicting the certificate drops the datagram under the bounded reason domain, and the attested label fills the sender's claims even with MQTTD_FAILURE_DOMAIN unset (relabel-by-reissue). Beyond self-assertion, the signed posture now accepts a member's label ONLY from the member itself: third-party label claims in relayed gossip are stripped (enforce_domain), closing relay-forged topology; labels converge via SWIM's direct probes. Damage-limiting also landed: run_driver warns (once per label) when a gossiped label overrides the static MQTTD_FAILURE_DOMAINS seed. Tests: mqtt-auth a_failure_domain_urn_in_the_san_is_returned, a_cert_without_the_urn_attests_no_domain, an_empty_failure_domain_label_is_rejected; swim_driver enforce_domain unit tests (third-party strip, attested override/fill, contradicting claim rejected, unattested claim kept); over-UDP a_cert_attested_domain_propagates_without_any_self_claim, a_domain_claim_contradicting_the_certificate_is_rejected."
  - id: 0016-T7
    title: Membership claims carry a process GENERATION — a restarted node is a new life, so a Dead claim about its previous one can neither kill it nor be re-gossiped (issue #92)
    status: done
    date: 2026-08-06
    evidence: "Shipped in e39b6e1 (#94) with the BASELINE_REF bump in 833ea5a (#95); recorded 2026-08-07 in a delivery-record audit. A rolling restart left a node NotReady FOREVER: it flapped alive<->dead in its peers' views every ~30s, each Dead dropped the peer link and its routing state, the durable lease group never elected, and readiness (which needs lease_group_ready) never returned. The cluster kept serving on the surviving quorum so nothing alerted — a three-node fleet silently ran on two, with no quorum margin, after the most routine operation there is. Three facts composed: incarnations are per-process and not persisted so a restarted node re-enters at the bottom of the range (and a k8s pod keeps its id); Dead outranks Alive at equal incarnation; and a claim NEVER DRAINS because applying one re-gossips it (Join -> gossip_full_state re-broadcasts every Dead a responder still holds). dead_ttl_ms = 30_000 supplied the metronome — exactly the observed period. ADR 0016 section 1 had considered the restart case and dismissed it (a node that genuinely restarts rejoins after the tombstone is pruned, it does not need to out-race stale gossip); it does, and it always loses, because the gossip it must outrun is regenerated by the act of losing. Claims now carry the subject's process generation: a LOWER generation is discarded outright and not re-gossiped (cutting the loop's fuel), a HIGHER one supersedes everything about the older life INCLUDING its tombstone (a restart is not a resurrection), and at the SAME generation every rule applies unchanged so a dead process's own in-flight refutation is still fenced. Two adjacent defects fixed with it: an address change that did not alter state raised no membership event (a dialer kept its spawn-time address for the life of the process), and a peer first seen through relayed gossip without a routing address was dropped for good. Guards: a_restarted_node_is_not_re_killed_by_its_previous_lifes_dead_claim (red before the change), a_tombstone_fences_the_dead_life_but_not_a_new_one, claims_about_a_previous_life_are_ignored_whatever_they_say. Post-mortem at docs/postmortems/2026-08-05-rolling-restart-membership-flap.md. Wire: Update.generation + Message.from_generation appended (pre-1.0 reshape, ADR 0039), so BASELINE_REF moved with it."
---
...
| **0016-T6** Attested labels | A cluster-bus cert carrying `URI:urn:fss:failure-domain:<label>` has its domain **CA-attested**: the verify path surfaces it and the driver enforces it over any self-claim (a contradicting claim drops the datagram); a member's label is accepted only from the member itself (third-party claims stripped); a gossiped label overriding the static seed warns once per label. |
...
| 0016-T6 | ✅ done | 2026-07-02 | "Option (1) of the recorded analysis (CA-attested labels), landed together with 0022-T7 (shared PKI surface). A cluster-bus leaf carrying SAN URI:urn:fss:failure-domain:<label> has its domain attested by the CA: signed_gossip::verify extracts it (a present-but-empty label is rejected as malformed, not ignored) and swim_driver enforces it beside the CN<->from binding — a self-claim (header or self-update) contradicting the certificate drops the datagram under the bounded reason domain, and the attested label fills the sender's claims even with MQTTD_FAILURE_DOMAIN unset (relabel-by-reissue). Beyond self-assertion, the signed posture now accepts a member's label ONLY from the member itself: third-party label claims in relayed gossip are stripped (enforce_domain), closing relay-forged topology; labels converge via SWIM's direct probes. Damage-limiting also landed: run_driver warns (once per label) when a gossiped label overrides the static MQTTD_FAILURE_DOMAINS seed. Tests: mqtt-auth a_failure_domain_urn_in_the_san_is_returned, a_cert_without_the_urn_attests_no_domain, an_empty_failure_domain_label_is_rejected; swim_driver enforce_domain unit tests (third-party strip, attested override/fill, contradicting claim rejected, unattested claim kept); over-UDP a_cert_attested_domain_propagates_without_any_self_claim, a_domain_claim_contradicting_the_certificate_is_rejected." |
| 0016-T7 | ✅ done | 2026-08-06 | "Shipped in e39b6e1 (#94) with the BASELINE_REF bump in 833ea5a (#95); recorded 2026-08-07 in a delivery-record audit. A rolling restart left a node NotReady FOREVER: it flapped alive<->dead in its peers' views every ~30s, each Dead dropped the peer link and its routing state, the durable lease group never elected, and readiness (which needs lease_group_ready) never returned. The cluster kept serving on the surviving quorum so nothing alerted — a three-node fleet silently ran on two, with no quorum margin, after the most routine operation there is. Three facts composed: incarnations are per-process and not persisted so a restarted node re-enters at the bottom of the range (and a k8s pod keeps its id); Dead outranks Alive at equal incarnation; and a claim NEVER DRAINS because applying one re-gossips it (Join -> gossip_full_state re-broadcasts every Dead a responder still holds). dead_ttl_ms = 30_000 supplied the metronome — exactly the observed period. ADR 0016 section 1 had considered the restart case and dismissed it (a node that genuinely restarts rejoins after the tombstone is pruned, it does not need to out-race stale gossip); it does, and it always loses, because the gossip it must outrun is regenerated by the act of losing. Claims now carry the subject's process generation: a LOWER generation is discarded outright and not re-gossiped (cutting the loop's fuel), a HIGHER one supersedes everything about the older life INCLUDING its tombstone (a restart is not a resurrection), and at the SAME generation every rule applies unchanged so a dead process's own in-flight refutation is still fenced. Two adjacent defects fixed with it: an address change that did not alter state raised no membership event (a dialer kept its spawn-time address for the life of the process), and a peer first seen through relayed gossip without a routing address was dropped for good. Guards: a_restarted_node_is_not_re_killed_by_its_previous_lifes_dead_claim (red before the change), a_tombstone_fences_the_dead_life_but_not_a_new_one, claims_about_a_previous_life_are_ignored_whatever_they_say. Post-mortem at docs/postmortems/2026-08-05-rolling-restart-membership-flap.md. Wire: Update.generation + Message.from_generation appended (pre-1.0 reshape, ADR 0039), so BASELINE_REF moved with it." |
<!-- /status-table:0016 -->
...
- **2026-07-02** — T6 (CA-attested labels) landed the same day its analysis was recorded,
  choosing option (1) and shipping it together with 0022-T7 (gossip cert revocation) since
  they share the PKI surface. The cluster-bus certificate now carries the failure domain
  (`URI:urn:fss:failure-domain:<label>`), making the CA the authority for topology: the
  gossip driver drops a datagram whose self-claimed domain contradicts its certificate,
  strips third-party label claims (a member's label is accepted only from the member
  itself — closing relay-forged topology, a hole *beyond* self-assertion), and lets an
  attested cert label a node with no `MQTTD_FAILURE_DOMAIN` at all (relabel = reissue the
  cert). The gossip-vs-static-seed mismatch `warn!` landed too. Limits #4 and #5 of the
  ADR's §5 are thereby closed for attested clusters; unattested certs keep the documented
  self-assertion residual trust (adopt incrementally by reissuing certs with the URN).
