...
tasks:
  ...
  - id: 0039-T3
    ...
    status: done
    date: 2026-08-18
    evidence: "Two releases exist (v0.9.0, v0.9.1) and the machinery is pointed at them: BASELINE_REF pins the v0.9.1 release commit, so the scheduled nightly upgrade job (nightly.yml, cron 17 3 * * *, the only lane that runs cluster_upgrade --ignored) now rolls a real adjacent release pair — previous release <-> 1.0 line — one node at a time in BOTH directions under the acked-facts oracle. The 0058 SS E residual is covered too: a_rolled_back_binary_reads_a_newer_config_under_warn runs the REAL previous-release binary via --check-config against a config carrying a key from a newer minor — refused by default naming the key, validated under the documented MQTTD_CONFIG_UNKNOWN_KEYS=warn hatch. From here BASELINE_REF advances per the skew policy: previous minor within a major, gateway minor across majors (RELEASING.md)."
---
...
| 0039-T3 | ✅ done | 2026-08-18 | "Two releases exist (v0.9.0, v0.9.1) and the machinery is pointed at them: BASELINE_REF pins the v0.9.1 release commit, so the scheduled nightly upgrade job (nightly.yml, cron 17 3 * * *, the only lane that runs cluster_upgrade --ignored) now rolls a real adjacent release pair — previous release <-> 1.0 line — one node at a time in BOTH directions under the acked-facts oracle. The 0058 SS E residual is covered too: a_rolled_back_binary_reads_a_newer_config_under_warn runs the REAL previous-release binary via --check-config against a config carrying a key from a newer minor — refused by default naming the key, validated under the documented MQTTD_CONFIG_UNKNOWN_KEYS=warn hatch. From here BASELINE_REF advances per the skew policy: previous minor within a major, gateway minor across majors (RELEASING.md)." |
<!-- /status-table:0039 -->
