...
tasks:
  ...
  - id: 0046-T4
    ...
    status: done
    date: 2026-07-19
    evidence: "reload.rs: Reloader gains an optional ConfigSource {live, path, precheck, apply}; when attached, reload() re-loads the whole config file (defaults < file < MQTTD_* env), validates it, runs the binary's runtime precheck, and swaps it into the shared live cell BEFORE rebuilding the policy — which reads paths from live, so a changed ACL/password/JWT path (not just file contents) takes effect. Validate-before-swap end to end: an invalid config, a precheck failure, or a failed policy build all roll live back and keep the running config. main.rs: the running config is an Arc<RwLock<Config>>; client_policy's build closures snapshot it each reload; attach_config_source wires precheck=runtime_precheck (the same fallible derivations startup runs — wire/queue limits, quotas, admission caps, watermark) and apply=apply_live_config (pushes SetQuotas + logs/audits every changed non-live section as requires-restart). Live-swappable: policy files, allow_anonymous, the three state quotas. Requires-restart (masked in requires_restart(), logged + audited 'config.reload'): listeners, tls paths, cluster/SWIM, durable, wire limits, observability, runtime, node. The config file is added to the MQTTD_CONFIG_WATCH set so a ConfigMap edit auto-reloads. Metered via the existing security_reloads_total{outcome,trigger}. Tests: reload.rs a_config_reload_swaps_live_and_keeps_it_on_any_failure (swap + reject-invalid + reject-precheck + rollback-on-policy-fail); main.rs requires_restart_masks_the_live_swappable_fields + runtime_precheck_rejects_what_startup_would_reject. Verified end-to-end with a live SIGHUP: max_sessions 100→200 applied silently, an added ws_bind flagged 'requires RESTART sections=listeners' + audited. mqttd lib 147 + bin 6 + check_config 5 green; clippy -D warnings + fmt clean."
  - id: 0046-T5
    ...
    status: done
    date: 2026-07-19
    evidence: "Secrets by reference: the last inline secret in the auth path — the JWT HS256 shared secret — is now read from a file (MQTTD_JWT_HS256_SECRET_FILE / security.jwt.hs256_secret_file, trailing newline trimmed), like the RS256 PEM; it joins the watched-policy-files set so a rotated secret hot-reloads, and is a path (not redacted) in the effective-config log. The gossip key gains a by-reference form: MQTTD_SWIM_KEY_FILE / cluster.swim.key_file reads the 64-hex key from a mounted file, mutually exclusive with the inline MQTTD_SWIM_KEY (validate() rejects both; inline stays for the harness/backward compat). So every secret — TLS keys, password file, JWT HS/RS keys, gossip key — can be referenced by path and kept out of a committed config file. ENV_VARS surface → 58 (HS256 renamed to *_FILE, SWIM_KEY_FILE added); bijection test updated. Docs: README Configuration section rewritten to lead with the file (precedence, --config/MQTTD_CONFIG, strict schema, --check-config, hot reload, secrets-by-reference), HS256/gossip-key/CONFIG rows updated; docs/mqttd.example.toml is a fully-commented template that `--check-config` validates clean (dogfooded); Dockerfile documents file + env + secret-by-path usage. Verified end-to-end: HS256 loads from file ('secret from file'), key_file shows as a path in the log, key+key_file rejected. ADR 0046 → Accepted (all five tasks done). mqtt-config 17 + mqttd lib 147 + bin 6 + check_config 5 green; clippy -D warnings + fmt clean."
  - id: 0046-T6
    title: Reject unknown or malformed CLI invocations before dispatch, configuration, listeners, storage or process signals; preserve documented commands and list their options in help
    status: done
    date: 2026-09-07
    issue: 544
    evidence: "Merged in PR #588 (e9eb312), all CI checks green: main.rs validates complete CLI shape before help/version or other command dispatch. Two pure parser tests cover valid invocations and rejected shapes; binary_smoke tests enforce exit 2 before config/storage startup and preserve successful help/version aliases. Bypassing the preflight makes the process regression fail with configuration loading and exit 1 instead of argument rejection. Binary unit, binary_smoke, check_config, probe, decommission and backup_restore suites passed locally; full CI passed before merge. Issue #544 closed after the code landed."
    notes: "The original task description was stale: #169 had already added help/version and dash-prefixed unknown-flag rejection. Remaining gaps were positional arguments, unscoped value options, duplicate options, conflicting commands, and help/version bypassing validation. No new CLI dependency, wire format or storage format."
---
...
| 0046-T4 | ✅ done | — | 2026-07-19 | "reload.rs: Reloader gains an optional ConfigSource {live, path, precheck, apply}; when attached, reload() re-loads the whole config file (defaults < file < MQTTD_* env), validates it, runs the binary's runtime precheck, and swaps it into the shared live cell BEFORE rebuilding the policy — which reads paths from live, so a changed ACL/password/JWT path (not just file contents) takes effect. Validate-before-swap end to end: an invalid config, a precheck failure, or a failed policy build all roll live back and keep the running config. main.rs: the running config is an Arc<RwLock<Config>>; client_policy's build closures snapshot it each reload; attach_config_source wires precheck=runtime_precheck (the same fallible derivations startup runs — wire/queue limits, quotas, admission caps, watermark) and apply=apply_live_config (pushes SetQuotas + logs/audits every changed non-live section as requires-restart). Live-swappable: policy files, allow_anonymous, the three state quotas. Requires-restart (masked in requires_restart(), logged + audited 'config.reload'): listeners, tls paths, cluster/SWIM, durable, wire limits, observability, runtime, node. The config file is added to the MQTTD_CONFIG_WATCH set so a ConfigMap edit auto-reloads. Metered via the existing security_reloads_total{outcome,trigger}. Tests: reload.rs a_config_reload_swaps_live_and_keeps_it_on_any_failure (swap + reject-invalid + reject-precheck + rollback-on-policy-fail); main.rs requires_restart_masks_the_live_swappable_fields + runtime_precheck_rejects_what_startup_would_reject. Verified end-to-end with a live SIGHUP: max_sessions 100→200 applied silently, an added ws_bind flagged 'requires RESTART sections=listeners' + audited. mqttd lib 147 + bin 6 + check_config 5 green; clippy -D warnings + fmt clean." |
| 0046-T5 | ✅ done | — | 2026-07-19 | "Secrets by reference: the last inline secret in the auth path — the JWT HS256 shared secret — is now read from a file (MQTTD_JWT_HS256_SECRET_FILE / security.jwt.hs256_secret_file, trailing newline trimmed), like the RS256 PEM; it joins the watched-policy-files set so a rotated secret hot-reloads, and is a path (not redacted) in the effective-config log. The gossip key gains a by-reference form: MQTTD_SWIM_KEY_FILE / cluster.swim.key_file reads the 64-hex key from a mounted file, mutually exclusive with the inline MQTTD_SWIM_KEY (validate() rejects both; inline stays for the harness/backward compat). So every secret — TLS keys, password file, JWT HS/RS keys, gossip key — can be referenced by path and kept out of a committed config file. ENV_VARS surface → 58 (HS256 renamed to *_FILE, SWIM_KEY_FILE added); bijection test updated. Docs: README Configuration section rewritten to lead with the file (precedence, --config/MQTTD_CONFIG, strict schema, --check-config, hot reload, secrets-by-reference), HS256/gossip-key/CONFIG rows updated; docs/mqttd.example.toml is a fully-commented template that `--check-config` validates clean (dogfooded); Dockerfile documents file + env + secret-by-path usage. Verified end-to-end: HS256 loads from file ('secret from file'), key_file shows as a path in the log, key+key_file rejected. ADR 0046 → Accepted (all five tasks done). mqtt-config 17 + mqttd lib 147 + bin 6 + check_config 5 green; clippy -D warnings + fmt clean." |
| 0046-T6 | ✅ done | [#544](https://github.com/mbilling/fss-mqtt-broker/issues/544) | 2026-09-07 | "Merged in PR #588 (e9eb312), all CI checks green: main.rs validates complete CLI shape before help/version or other command dispatch. Two pure parser tests cover valid invocations and rejected shapes; binary_smoke tests enforce exit 2 before config/storage startup and preserve successful help/version aliases. Bypassing the preflight makes the process regression fail with configuration loading and exit 1 instead of argument rejection. Binary unit, binary_smoke, check_config, probe, decommission and backup_restore suites passed locally; full CI passed before merge. Issue #544 closed after the code landed." |
<!-- /status-table:0046 -->
...
- **2026-07-19** — **T4 done.** Whole-config hot reload. `Reloader` gained an optional
  `ConfigSource`, so `SIGHUP` / the `MQTTD_CONFIG_WATCH` watcher now re-load the *config file*
  (not just the policy file contents at fixed paths) through the ADR 0032 validate-before-swap
  path. The running config lives in an `Arc<RwLock<Config>>` the policy build closures snapshot,
  so a changed ACL/password/JWT path — and `allow_anonymous`, and the state quotas — take effect
  live; every other change (listeners, cluster, durable, wire limits, …) is logged and audited as
  requires-restart. A bad edit (invalid config, failed precheck, or a broken referenced file)
  rolls the config back and keeps the running one — validate-before-swap end to end. Verified with
  a live `SIGHUP`: a `max_sessions` bump applied silently while an added `ws_bind` was flagged
  requires-restart. Metered via `security_reloads_total{outcome,trigger}`.
- **2026-07-19** — **T5 done; ADR Accepted.** Secrets by reference: the JWT HS256 secret (the last
  inline secret in the auth path) now reads from a file (`MQTTD_JWT_HS256_SECRET_FILE`, hot-reloadable,
  trailing newline trimmed), and the gossip key gains a by-reference form (`MQTTD_SWIM_KEY_FILE`,
  mutually exclusive with the inline `MQTTD_SWIM_KEY`) — so every secret can be path-referenced and
  mounted from a Secret, never inlined in a committed file. Docs: the README Configuration section now
  leads with the file (precedence, `--check-config`, hot reload, secrets-by-reference); a fully-commented
