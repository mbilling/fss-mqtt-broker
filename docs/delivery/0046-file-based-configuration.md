...
tasks:
  ...
  - id: 0046-T6
    ...
    status: in-progress
    issue: 544
    evidence: "Implementation prepared for review: main.rs validates complete CLI shape before help/version or other command dispatch. Two pure parser tests cover valid invocations and rejected shapes; binary_smoke tests enforce exit 2 before config/storage startup and preserve successful help/version aliases. Bypassing the preflight makes the process regression fail with configuration loading and exit 1 instead of argument rejection. Awaiting PR review and merge before marking delivered."
...
| 0046-T6 | 🚧 in-progress | [#544](https://github.com/mbilling/fss-mqtt-broker/issues/544) | — | "Implementation prepared for review: main.rs validates complete CLI shape before help/version or other command dispatch. Two pure parser tests cover valid invocations and rejected shapes; binary_smoke tests enforce exit 2 before config/storage startup and preserve successful help/version aliases. Bypassing the preflight makes the process regression fail with configuration loading and exit 1 instead of argument rejection. Awaiting PR review and merge before marking delivered." |
<!-- /status-table:0046 -->
