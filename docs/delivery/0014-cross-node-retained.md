...
tasks:
  ...
  - id: 0014-T7
    ...
    status: done
    date: 2026-07-04
    evidence: "Closed by ADR 0037 (Accepted, all phases delivered): divergence is PREVENTED rather than reconciled — retained mutations commit through the topic's placement-group lease-owner with clock-free (epoch, offset) convergence tokens (LWW/HLC reconciliation was considered and rejected: clocks in the trust base, silently dropped acked writes). Both divergence classes this task tracked are proven closed by integration tests over real cluster transports: the EVERYDAY RACE (0037-P4, concurrent_retained_publishes_on_two_nodes_converge_cluster_wide — two nodes publish different values to one topic concurrently on the full durable stack and converge cluster-wide with retained_divergence_total 0) and the PARTITION HEAL (0037-P6, divergent_retained_writes_across_a_partition_converge_after_heal — the majority side commits, the minority side queues-until-heal and serves the last committed value, and on heal every node converges to the queued write committed through the owner). Cache convergence machinery: post-commit token fan-out (0037-P4) + higher-token-wins back-fill replacing gap-fill, with committed clears as tombstone entries (0037-P5). ADR 0014 carries revision notes; durable-off keeps the original ADR 0014 behaviour, divergence caveat included."
  - id: 0014-T8
  ...
  - id: 0014-T10
    title: The retained digest also runs on a CADENCE, not only at link-up — an update lost on a link that stays up becomes self-healing rather than permanent (issue #87 item 1)
    status: done
    date: 2026-08-06
    evidence: "Shipped in 380f58a (#96); recorded 2026-08-07 in a delivery-record audit. Section 3's digest was specified as a link-up exchange, on the reasoning that a commit's fan-out delivers the update and the digest need only cover what a new or reconnected link missed. That left NO reconciliation for an update lost on a link that stays up — and one can be: a commit fans out exactly ONE frame, unacked and never retransmitted, and an oversized or unencodable frame is dropped with the link deliberately kept. The result was a peer silently diverging on retained state until something flapped its link, which in a healthy cluster may be never. The same digest is now offered to every peer every ~30s (RETAINED_ANTIENTROPY_EVERY). The exchange is unchanged and idempotent: a peer whose digest matches early-returns and transfers nothing, so the steady state is one small frame per peer per period; the digest is computed once and fanned out, so a period costs one retained scan regardless of peer count — the same scan a link-up already pays; a node holding no retained state stays silent. Guard: a_missed_retained_fan_out_is_healed_by_periodic_anti_entropy, verified RED with the broadcast disabled, plus a sibling asserting the cadence says nothing when there is nothing retained. ADR 0014 amended. NOTE this does NOT heal issue #87 item 2 (token recorded before the store write): a node holding a token with no value skips the repairing snapshot because the token is not strictly newer — tracked separately."
---
...
| 0014-T7 | ✅ done | 2026-07-04 | "Closed by ADR 0037 (Accepted, all phases delivered): divergence is PREVENTED rather than reconciled — retained mutations commit through the topic's placement-group lease-owner with clock-free (epoch, offset) convergence tokens (LWW/HLC reconciliation was considered and rejected: clocks in the trust base, silently dropped acked writes). Both divergence classes this task tracked are proven closed by integration tests over real cluster transports: the EVERYDAY RACE (0037-P4, concurrent_retained_publishes_on_two_nodes_converge_cluster_wide — two nodes publish different values to one topic concurrently on the full durable stack and converge cluster-wide with retained_divergence_total 0) and the PARTITION HEAL (0037-P6, divergent_retained_writes_across_a_partition_converge_after_heal — the majority side commits, the minority side queues-until-heal and serves the last committed value, and on heal every node converges to the queued write committed through the owner). Cache convergence machinery: post-commit token fan-out (0037-P4) + higher-token-wins back-fill replacing gap-fill, with committed clears as tombstone entries (0037-P5). ADR 0014 carries revision notes; durable-off keeps the original ADR 0014 behaviour, divergence caveat included." |
...
| 0014-T10 | ✅ done | 2026-08-06 | "Shipped in 380f58a (#96); recorded 2026-08-07 in a delivery-record audit. Section 3's digest was specified as a link-up exchange, on the reasoning that a commit's fan-out delivers the update and the digest need only cover what a new or reconnected link missed. That left NO reconciliation for an update lost on a link that stays up — and one can be: a commit fans out exactly ONE frame, unacked and never retransmitted, and an oversized or unencodable frame is dropped with the link deliberately kept. The result was a peer silently diverging on retained state until something flapped its link, which in a healthy cluster may be never. The same digest is now offered to every peer every ~30s (RETAINED_ANTIENTROPY_EVERY). The exchange is unchanged and idempotent: a peer whose digest matches early-returns and transfers nothing, so the steady state is one small frame per peer per period; the digest is computed once and fanned out, so a period costs one retained scan regardless of peer count — the same scan a link-up already pays; a node holding no retained state stays silent. Guard: a_missed_retained_fan_out_is_healed_by_periodic_anti_entropy, verified RED with the broadcast disabled, plus a sibling asserting the cadence says nothing when there is nothing retained. ADR 0014 amended. NOTE this does NOT heal issue #87 item 2 (token recorded before the store write): a node holding a token with no value skips the repairing snapshot because the token is not strictly newer — tracked separately." |
<!-- /status-table:0014 -->
