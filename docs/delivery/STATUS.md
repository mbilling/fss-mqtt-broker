...
- `0077-T4` 🚧 in-progress: "Sites per node — is the tenant capacity of a cluster linear in its nodes?" — "RUN DONE 2026-08-29 (v1.0.10, sizes 3 and 5, ladder 1/2/4/8, clean teardown) but the HEADLINE QUESTION IS NOT ANSWERED, and the reason is this task's own ladder rather than anything measured. Same-rung results, which is the reading the data supports: at 4 sites / 120,000 msg/s p99 went <=2000ms (N=1) -> <=1000ms (N=3) -> <=1ms (N=5), all delivering ~100%; at the saturated 8-site rung delivery went 24.0% -> 55.5% -> 82.3% (57,600 -> 133,200 -> 197,520 msg/s, i.e. 57.6k -> 44.4k -> 39.5k per node). So added nodes help substantially and consistently, sub-linearly at the saturated rung. WHY THE HEADLINE FAILS: the summarizer reports '4 sites' for BOTH N=3 and N=5, which looks like a plateau and is not one — the ladder simply has no rung between 4 and 8. At N=5 the 4-site rung ran at p99 <=1ms with the hottest core at 78.5%, nowhere near saturation, so the true knee is somewhere in (4,8]. The derived 'sites per node' figures (2.0 at N=1, 1.3 at N=3, 0.8 at N=5) are ladder artifacts and must NOT be quoted: a doubling ladder locates a knee only to an interval, and the intervals for the three sizes overlap. The ladder was designed to find one knee cheaply, which it does; comparing knees ACROSS sizes needs resolution it does not have. MECHANISM CONFIRMED at every size: the box never saturates, one core does. At the failing rung the hottest core sat at 96-97% while aggregate broker CPU was 76-80% (N=5: 49.4% aggregate / 78.5% hot at 4 sites, 75.9% / 96.9% at 8). The ADR 0048/#447 single hub thread is PER-NODE and remains the ceiling at 1, 3 and 5 nodes alike — which is why more nodes give sub-linear returns and why #447 is the lever no amount of horizontal scaling substitutes for. TO FINISH: one run per size with LANE_E_SITES_OVERRIDE="4 5 6 7 8" at N=3 and N=5 (plus "2 3" at N=1) resolves all three knees to a single site and makes the linearity claim quotable. Roughly 2-3 provisionings, EUR 3-5. Until then T4 has a direction but no purchasable number."
