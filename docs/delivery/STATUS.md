...
| [0077](../adr/0077-workload-targeted-performance.md) | # 0077. Workload-targeted performance — a shape is not a result without its tail | Proposed | [3/9 done](0077-workload-targeted-performance.md) | 6 open |
...
- `0077-T9` ⬜ planned: "The driver-side `sent` counter reads 2x the offered rate, and it silently suppresses knee detection" — "Found while reading T7. lane_b_rung's sent_rate is exactly 2.00x the labelled rate on EVERY rung of the T7 probe AND of market-data v1.0.10 — systematic across the rig, not specific to one shape. It is not catch-up bursting (pub_overrun is 0 at those rungs) and driver_rate correctly differences totals over the steady window. The broker's received counter matches the LABEL, not the doubled figure: market-data's measured TPS only reconciles if the broker received 4,800/s rather than 9,600. So the labels are right and one driver-side counter is wrong. It matters because `sustained` is recv >= KNEE_OK * sent, so a doubled sent marks every rung unsustained — summarize-curve.py reported 'NO sustained rung' for the T7 run in which three rungs tracked their offer exactly. Every lane B knee this rig has ever printed is suspect until this is explained."
