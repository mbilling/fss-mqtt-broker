...
- `0047-T5` 🚧 in-progress: Quorum-safe rollout — StatefulSet one-at-a-time RollingUpdate (ADR 0039 motion) + PodDisruptionBudget maxUnavailable 1; a kind/k3d smoke test in CI stands up a cluster, scales, and rolls, asserting no acked loss — "Rollout config authored in the chart: StatefulSet OrderedReady + RollingUpdate (one pod at a time, each rejoining before the next — ADR 0039), and a PodDisruptionBudget maxUnavailable 1 (a node drain can't take two brokers / quorum). A CI `helm` job lints + templates + kubeconform-validates the whole chart offline. The live runtime smoke is authored: scripts/k8s/kind-smoke.sh + deploy/helm/mqttd/ci/values-smoke.yaml + a nightly `kube-smoke` job stand the chart up as a real 3-node cluster in kind, prove it forms (founder bootstrap + gossip mesh + per-pod PV + rendered config + check-config init + readiness), publish a durable retained message, scale down 3→2 (the departing pod drains via preStop `mqttd --decommission`), and rolling-restart — asserting the retained state survives both. Awaiting its first green nightly/workflow_dispatch run before this flips to done (the sandbox has no kind/docker to run it locally; it validates in GitHub Actions)."
