...
- `0048-T3` ⬜ planned: The scaling curve — the same workload against 1/3/5 nodes, throughput and p99 vs node count; tests capability claim 1 and the ADR 0015 shared-subscription mechanism end to end; a flat curve is a finding to fix — "2026-08-20: the rig and method are BUILT, the run is not yet paid for. bench/scale/ provisions fresh Hetzner clusters per size (Terraform + cloud-init, one CCX23 + local NVMe per broker in a spread placement group, drivers separate; §2's hardware rule by construction), boots them founder-first from the shipped systemd artifacts via deploy/systemd/gen-certs.sh, runs per-host barrier probes as a gate, then three lanes: durable QoS1 closed-loop (durable_bench, new MQTTD_BENCH_SPREAD=1 spreads session ownership across all nodes — the curve's shape), a $share/g1 fan-out ladder on emqtt-bench (the ADR 0015 mechanism), and 50k idle connections. summarize-curve.py enforces the honesty mechanics (probe-gated Curve 1, bucket-bound p99 labeling, ±2% counter cross-checks, driver-limited rung exclusion). docs/benchmarks/SCALE-CURVE.md is the record's template, committed empty on purpose. First finding arrived before any spend: spread ownership stalls durable acks on non-founder-owned groups (issue #358) — 3/5-node durable points would currently measure that defect."
