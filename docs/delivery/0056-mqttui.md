...
tasks:
  ...
  - id: 0056-T7
    ...
    status: done
    date: 2026-08-10
    evidence: "tools/mqttui/src/embedded.rs — demo/, deploy/, scripts/migrate/ and scripts/k8s/ travel inside the binary via include_dir!, unpacked once to ~/.cache/mqttui/examples-<version> behind a .complete stamp so an interrupted unpack is redone rather than trusted, with .sh files chmod +x because otherwise every standalone task fails at exec. The manifest travels too (manifest::EMBEDDED). Verified by running the release binary from a temp directory outside any checkout: the Mosquitto converter reads a user's own mosquitto.conf and emits a config, and `--list` marks what it cannot run."
    notes: "MEASURED: 161 KB compressed, so include_dir! costs nothing worth discussing. Embedding rather than fetching buys offline operation, version-locking to the binary that was tested with it, and — the load-bearing property — executing nothing that arrived over the network. TWO DEFECTS FOUND BY RUNNING IT, not by reading it. (1) Availability was inferred from whether the script file was present, so k8s-render-parity showed as available, RAN, and died with `could not find Cargo.toml` — it is embedded and still unrunnable, because what it needs is the operator crate. That is the same discovery mistake §2 rejects, so `needs_checkout` is now DECLARED in tasks.toml for the nine tasks that operate on the repository. Unavailability also has two distinct reasons — needs-a-clone versus not-bundled — kept apart because telling someone to clone when the benchmark harness merely did not travel sends them to do work that will not help. (2) migrate-mosquitto declared INPUT as 'passed as the first argument' while the runner only ever exported it, so the task could not work as described; the manifest gained a declared `args` field with ${VAR} substitution, and the headless path now resolves environment through the same env.rs function the UI uses instead of its own slightly different rule."
  - id: 0056-T8
    ...
    status: done
    date: 2026-08-10
    evidence: ".github/workflows/examples-bundle.yml publishes, on every merge to main, a tarball of the vendored bundle + tasks.toml + a BUNDLE_PROVENANCE stamp to the rolling `examples-main` prerelease, cosign-signed keylessly. tools/mqttui/src/update.rs downloads it and verifies BEFORE unpacking a byte: the signing identity (this repo's workflows) and OIDC issuer are hard-coded constants — verification has no configuration surface, because a configurable verifier is a disableable one. Only the download URL is overridable (MQTTUI_UPDATE_BASE_URL, for tests), which is safe precisely because a different server cannot mint a certificate for this repo's identity. An installed update takes precedence standalone, carries its own manifest (so tasks declared after the binary shipped still appear; serde ignores unknown fields the other way), and `--list` LEADS with its provenance. `--channel main` fetches the raw branch tarball: explicit, warned at install AND restated on every list — a warning printed once is a warning forgotten. `--clear` returns to the embedded copy. curl/cosign/tar are shelled out like every task this tool runs; a missing verifier refuses by name rather than degrading into not verifying."
    notes: "Tested live: the unverified channel end-to-end (install → list leads with UNVERIFIED → a task runs from the updated bundle → --clear returns to embedded); garbage in place of a signature fails verification (real cosign, unit test); a hollow bundle missing its skeleton is refused rather than installed as a silently shrunken list; missing tools refuse by name. The signed path's download/verify sequencing is the same code path, but its end-to-end run needs the rolling release to exist — CI publishes the first bundle on the merge of this very PR, so `mqttui update` against the real release was verified immediately AFTER merge rather than before. A fourth entry-point defect found while wiring provenance in: the TUI never learned T7's availability rules — standalone it offered repo-only tasks and let them die mid-run; it now marks them `-`, dims them, and refuses with the reason, same as --list/--run."
  - id: 0056-T9
    title: Distribution — signed musl binaries through the ADR 0045 pipeline (primary) plus crates.io (`cargo install mqttui`)
    status: done
    date: 2026-08-10
    evidence: "scripts/release/build-repro.sh builds mqttui through its OWN manifest so --locked binds to its own lockfile, under the same recipe (same rustc, same path remapping, same SOURCE_DATE_EPOCH) as the broker — so it is reproducible and verifiable identically. release.yml builds, reproducibility-checks, checksums, provenance-attests and cosign-signs it as a third binary on both musl targets, and cuts it a CycloneDX SBOM from its own lockfile. ci.yml gained `cargo publish --dry-run` and the vendored-bundle check. RELEASING.md documents the manual crates.io step. VERIFIED: built for aarch64-unknown-linux-musl in a Linux container — 2.1 MB, `not a dynamic executable`, runs; and `cargo install` from the packaged tarball into a clean root, then run from a directory containing no repository, unpacked the bundle and ran a real script."
    notes: "PUBLISHING BROKE THE EMBEDDING, and only running it showed that: `cargo package` includes only files beneath the package root, so include_dir! pointing at ../../demo builds from a checkout and produces a crate that cannot compile once published — `cargo publish --dry-run` failed with '.../mqttui-0.1.0/../../scripts/k8s is not a directory'. tools/mqttui/bundle/ is now a generated copy (scripts/vendor-mqttui-examples.sh, 49 files, 190 KB compressed) with the originals still the source of truth and a CI-gated --check; both directions of drift were mutation-tested. The reproducibility step had a check that could not fail: `cargo clean` at the root does not touch tools/mqttui/target, so the rebuild would have re-checksummed the same file — both workspaces are cleaned now, and the step asserts the binaries are actually gone before rebuilding. mqttui is deliberately NOT added to the container images (a TUI in a distroless image is attack surface no user can reach) and the crates.io publish stays manual (a published version is permanent). A THIRD defect surfaced from running the installed binary: demo-quic-certs was offered as 'run once before demo-stack' but its CERT_DIR defaults to /certs, a path inside the demo container, and the compose stack already mints the PKI in a named volume via an init service eight brokers depend on — it failed with `mkdir: /certs: Read-only file system` on any host, checkout or not. Now declared hidden with an accurate description."
  - id: 0056-T10
...
| 0056-T7 | ✅ done | 2026-08-10 | "tools/mqttui/src/embedded.rs — demo/, deploy/, scripts/migrate/ and scripts/k8s/ travel inside the binary via include_dir!, unpacked once to ~/.cache/mqttui/examples-<version> behind a .complete stamp so an interrupted unpack is redone rather than trusted, with .sh files chmod +x because otherwise every standalone task fails at exec. The manifest travels too (manifest::EMBEDDED). Verified by running the release binary from a temp directory outside any checkout: the Mosquitto converter reads a user's own mosquitto.conf and emits a config, and `--list` marks what it cannot run." |
| 0056-T8 | ✅ done | 2026-08-10 | ".github/workflows/examples-bundle.yml publishes, on every merge to main, a tarball of the vendored bundle + tasks.toml + a BUNDLE_PROVENANCE stamp to the rolling `examples-main` prerelease, cosign-signed keylessly. tools/mqttui/src/update.rs downloads it and verifies BEFORE unpacking a byte: the signing identity (this repo's workflows) and OIDC issuer are hard-coded constants — verification has no configuration surface, because a configurable verifier is a disableable one. Only the download URL is overridable (MQTTUI_UPDATE_BASE_URL, for tests), which is safe precisely because a different server cannot mint a certificate for this repo's identity. An installed update takes precedence standalone, carries its own manifest (so tasks declared after the binary shipped still appear; serde ignores unknown fields the other way), and `--list` LEADS with its provenance. `--channel main` fetches the raw branch tarball: explicit, warned at install AND restated on every list — a warning printed once is a warning forgotten. `--clear` returns to the embedded copy. curl/cosign/tar are shelled out like every task this tool runs; a missing verifier refuses by name rather than degrading into not verifying." |
| 0056-T9 | ✅ done | 2026-08-10 | "scripts/release/build-repro.sh builds mqttui through its OWN manifest so --locked binds to its own lockfile, under the same recipe (same rustc, same path remapping, same SOURCE_DATE_EPOCH) as the broker — so it is reproducible and verifiable identically. release.yml builds, reproducibility-checks, checksums, provenance-attests and cosign-signs it as a third binary on both musl targets, and cuts it a CycloneDX SBOM from its own lockfile. ci.yml gained `cargo publish --dry-run` and the vendored-bundle check. RELEASING.md documents the manual crates.io step. VERIFIED: built for aarch64-unknown-linux-musl in a Linux container — 2.1 MB, `not a dynamic executable`, runs; and `cargo install` from the packaged tarball into a clean root, then run from a directory containing no repository, unpacked the bundle and ran a real script." |
...
- **2026-08-10** — **`mqttui` becomes an installed product, not a checkout tool** (ADR
  ...
  The example surface measures **190 KB compressed**, so it is embedded rather than
