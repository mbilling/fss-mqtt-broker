...
tasks:
  ...
  - id: 0031-T5
    ...
    status: planned
    notes: "The optional, opt-in refinement (ADR option B), layered on the shipped secure-by-default guard. Not required for the decision; deferred until a deployment needs per-identity client-id namespacing as policy. Will add Authorizer::authorize_connect (default-allow) + ACL connect-rule syntax + a CONNECT-time check."
  - id: 0031-T6
...
and proven (T1–T4, T6); the optional connect ACL (T5) is a tracked opt-in refinement.
...
| 0031-T5 | ⬜ planned | — | "The optional, opt-in refinement (ADR option B), layered on the shipped secure-by-default guard. Not required for the decision; deferred until a deployment needs per-identity client-id namespacing as policy. Will add Authorizer::authorize_connect (default-allow) + ACL connect-rule syntax + a CONNECT-time check." |
