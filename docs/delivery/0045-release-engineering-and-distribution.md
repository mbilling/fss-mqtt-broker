...
tasks:
  ...
  - id: 0045-T6
    title: The image's data directory — a writable /var/lib/mqttd owned by the nonroot uid, so durable sessions can be enabled in a container at all, plus an image smoke covering nonroot writes, restart durability and a read-only root filesystem
    status: done
    date: 2026-08-08
    evidence: "Walking the evaluation path with nothing but Docker found the image's headline feature unreachable: distroless/static:nonroot runs as uid 65532 and the image shipped NO directory that uid could write, so durable sessions — enabled by default — could not be turned on in a container. MQTTD_DATA_DIR=/data failed 'create data dir /data: Permission denied'; a named volume failed 'stamp data dir'; a subdirectory failed the same way. The only ways through were --user 0:0, discarding the very non-root posture the image exists to provide, or a world-writable host path. FIX: the Dockerfile COPYs deploy/image/data-dir/ to /var/lib/mqttd with --chown=65532:65532. COPY rather than mkdir because distroless has no shell, and WORKDIR/VOLUME would create it root-owned; the ownership matters for volumes specifically, since Docker seeds an empty named volume from the image path it covers, ownership included, so -v mqttd-data:/var/lib/mqttd now lands writable. MQTTD_DATA_DIR is deliberately NOT defaulted — unset still means in-memory, exactly as the bare binary behaves. GUARD (scripts/image-smoke.sh, nightly image-smoke job): every other test in this repo drives the BINARY, which is why nothing caught this; the smoke runs the IMAGE and asserts it runs as nonroot, that MQTTD_DATA_DIR on a mounted volume is writable, that durable state survives a container restart, and that it serves under --read-only --cap-drop ALL --security-opt no-new-privileges. Verified falsifiable against the real defect: pointed at ghcr.io/mbilling/fss-mqtt-broker:0.9.0-rc it fails with 'stamp data dir /var/lib/mqttd: Permission denied'. README documents the hardened run command and states that persistence needs both the env var and a volume."
  - id: 0045-T5
    ...
    status: done
    date: 2026-08-11
    evidence: "release.yml generates a CycloneDX SBOM per shipped binary from the LOCKED source graph (cargo-cyclonedx, lockfile-drift guarded), cosign-signed. VERIFIED on the real release: v0.9.0 carries sbom-0.9.0.cdx.json + .sig + .pem among its 15 signed assets (gh release view v0.9.0). mqttui gained its own lockfile-cut SBOM in the T9 distribution work."
    notes: "The 'remaining: cut the first 0.x release' clause is what kept this in-progress; v0.9.0 shipped 2026-08-07, so it is now done. The SBOM + cargo-deny/cargo-audit gate + RELEASING/README verify docs were already in place."
---
...
| 0045-T6 | ✅ done | 2026-08-08 | "Walking the evaluation path with nothing but Docker found the image's headline feature unreachable: distroless/static:nonroot runs as uid 65532 and the image shipped NO directory that uid could write, so durable sessions — enabled by default — could not be turned on in a container. MQTTD_DATA_DIR=/data failed 'create data dir /data: Permission denied'; a named volume failed 'stamp data dir'; a subdirectory failed the same way. The only ways through were --user 0:0, discarding the very non-root posture the image exists to provide, or a world-writable host path. FIX: the Dockerfile COPYs deploy/image/data-dir/ to /var/lib/mqttd with --chown=65532:65532. COPY rather than mkdir because distroless has no shell, and WORKDIR/VOLUME would create it root-owned; the ownership matters for volumes specifically, since Docker seeds an empty named volume from the image path it covers, ownership included, so -v mqttd-data:/var/lib/mqttd now lands writable. MQTTD_DATA_DIR is deliberately NOT defaulted — unset still means in-memory, exactly as the bare binary behaves. GUARD (scripts/image-smoke.sh, nightly image-smoke job): every other test in this repo drives the BINARY, which is why nothing caught this; the smoke runs the IMAGE and asserts it runs as nonroot, that MQTTD_DATA_DIR on a mounted volume is writable, that durable state survives a container restart, and that it serves under --read-only --cap-drop ALL --security-opt no-new-privileges. Verified falsifiable against the real defect: pointed at ghcr.io/mbilling/fss-mqtt-broker:0.9.0-rc it fails with 'stamp data dir /var/lib/mqttd: Permission denied'. README documents the hardened run command and states that persistence needs both the env var and a volume." |
| 0045-T5 | ✅ done | 2026-08-11 | "release.yml generates a CycloneDX SBOM per shipped binary from the LOCKED source graph (cargo-cyclonedx, lockfile-drift guarded), cosign-signed. VERIFIED on the real release: v0.9.0 carries sbom-0.9.0.cdx.json + .sig + .pem among its 15 signed assets (gh release view v0.9.0). mqttui gained its own lockfile-cut SBOM in the T9 distribution work." |
<!-- /status-table:0045 -->
