...
tasks:
  ...
  - id: 0074-T3
    title: "The isolation falsifier for Decision 2: a deliberately stalled QoS 2 store must not stall unrelated sessions — synchronize on the stall, enqueue an unrelated command with a reply channel, assert the reply lands while the stall is held"
    status: in-progress
    date: 2026-09-12
    issue: 575
    evidence: "GREEN 2026-09-12: hub::a_parked_qos2_truncate_does_not_stall_unrelated_sessions is un-ignored and asserts isolation directly — synchronize on park_ack entering the stall, enqueue an unrelated attach (reply channel), assert the reply arrives while the stall is still held. Actor-path pub_rec/pub_comp/pub_ack and the QoS 2 sweep submit those store waits to the session's existing append lane (LaneJob::Qos2Op, control-job headroom, FIFO behind record_outbound). Completions return as HubCommand::Qos2OpDone; epoch fences discard/clean-start. Writes stay ordered: ack_durable prefix then clear_outbound (#577), never fire-and-forget (#533). Direct Hub method calls used by the #577 fixtures remain inline (no actor to drain the lane). Hygiene allowlist entry removed so CI runs the oracle. Not marked done while #575 is open (check-issue-alignment). Out of scope: inbound PUBREC/PUBREL pipelining, record+append fusion, the rest of #405."
---
...
| 0074-T3 | 🚧 in-progress | [#575](https://github.com/mbilling/fss-mqtt-broker/issues/575) | 2026-09-12 | "GREEN 2026-09-12: hub::a_parked_qos2_truncate_does_not_stall_unrelated_sessions is un-ignored and asserts isolation directly — synchronize on park_ack entering the stall, enqueue an unrelated attach (reply channel), assert the reply arrives while the stall is still held. Actor-path pub_rec/pub_comp/pub_ack and the QoS 2 sweep submit those store waits to the session's existing append lane (LaneJob::Qos2Op, control-job headroom, FIFO behind record_outbound). Completions return as HubCommand::Qos2OpDone; epoch fences discard/clean-start. Writes stay ordered: ack_durable prefix then clear_outbound (#577), never fire-and-forget (#533). Direct Hub method calls used by the #577 fixtures remain inline (no actor to drain the lane). Hygiene allowlist entry removed so CI runs the oracle. Not marked done while #575 is open (check-issue-alignment). Out of scope: inbound PUBREC/PUBREL pipelining, record+append fusion, the rest of #405." |
<!-- /status-table:0074 -->
...
- 2026-09-12 — T3 isolation slice: QoS 2 completion/advance store waits move
  onto the per-session append lane. The #575 oracle is un-ignored and green.
  Prefix-then-clear and packet-ID retirement are unchanged; full #405
  pipelining/fusion is not claimed. Task stays in-progress until #575 closes.
