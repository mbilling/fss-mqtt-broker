...
tasks:
  ...
  - id: 0074-T3
    ...
    date: 2026-09-07
    issue: 575
    notes: "The obvious test does NOT work and must not be used: Hub::run starts its dispatch timer AFTER rx.recv() returns, so a command that waited a second in the channel still reports a microsecond dispatch — the hub_dispatch histogram stays flat under a stall and reads as evidence of isolation. The reply-channel assertion is scheduling-independent, where a timing threshold would not be. Expected RED against today's inline path (ADR 0074 Decision 2 keeps the QoS 2 completion awaits on-loop deliberately); lands green with the #405 isolation work. Enqueue-timestamp telemetry is a separate concern — HubCommand carries none today. RED HALF LANDED 2026-09-07 (commit 5f1ace2): hub::a_parked_qos2_truncate_does_not_stall_unrelated_sessions, #[ignore]d as the reproduction — with one QoS 2 subscriber's truncate parked an unrelated session cannot attach AT ALL, so the blocking argued from the source across the review is now demonstrated. The task completes when it is un-ignored and green, which needs the #405 isolation work; do not force it green by detaching the writes — #533 has since REPRODUCED a QoS 2 duplicate in that same completion path. Raised by the 2026-09-06 scaling review.
---
...
| 0074-T3 | 🚧 in-progress | [#575](https://github.com/mbilling/fss-mqtt-broker/issues/575) | 2026-09-07 | "The obvious test does NOT work and must not be used: Hub::run starts its dispatch timer AFTER rx.recv() returns, so a command that waited a second in the channel still reports a microsecond dispatch — the hub_dispatch histogram stays flat under a stall and reads as evidence of isolation. The reply-channel assertion is scheduling-independent, where a timing threshold would not be. Expected RED against today's inline path (ADR 0074 Decision 2 keeps the QoS 2 completion awaits on-loop deliberately); lands green with the #405 isolation work. Enqueue-timestamp telemetry is a separate concern — HubCommand carries none today. RED HALF LANDED 2026-09-07 (commit 5f1ace2): hub::a_parked_qos2_truncate_does_not_stall_unrelated_sessions, #[ignore]d as the reproduction — with one QoS 2 subscriber's truncate parked an unrelated session cannot attach AT ALL, so the blocking argued from the source across the review is now demonstrated. The task completes when it is un-ignored and green, which needs the #405 isolation work; do not force it green by detaching the writes — #533 has since REPRODUCED a QoS 2 duplicate in that same completion path. Raised by the 2026-09-06 scaling review. |
<!-- /status-table:0074 -->
