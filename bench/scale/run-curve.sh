...
lane_a() { # lane_a <name> <publishers> <window> <subs>
	...
	with_cpu_sampling "$OUT/laneA/cpu-$name" rssh "$(driver_pub_ip 0)" \
		...
		>"$OUT/laneA/$name.txt" 2>&1 || die "lane A $name or CPU sampling failed — kept output"
...
lane_a_tier() { # lane_a_tier <tier>
	...
	with_cpu_sampling "$OUT/laneA/cpu-tier-$t" rssh "$(driver_pub_ip 0)" \
		"MQTTD_BENCH_BROKERS=$BROKERS_LIST MQTTD_BENCH_HEALTH=$HEALTH_LIST MQTTD_BENCH_NODE_IDS=$IDS_LIST \
		 ...
		 MQTTD_BENCH_REPS=$A_REPS $BIN_PATH durable_path_floor --ignored --nocapture" \
		>"$OUT/laneA/tier-$t.txt" 2>&1 || die "lane A tier $t or CPU sampling failed — kept output"
	...
	with_cpu_sampling "$OUT/laneA/cpu-tier-$t-lat" rssh "$(driver_pub_ip 0)" \
		"MQTTD_BENCH_BROKERS=$BROKERS_LIST MQTTD_BENCH_HEALTH=$HEALTH_LIST MQTTD_BENCH_NODE_IDS=$IDS_LIST \
		 ...
		 MQTTD_BENCH_REPS=$A_REPS $BIN_PATH durable_path_floor --ignored --nocapture" \
		>"$OUT/laneA/tier-$t-lat.txt" 2>&1 || die "lane A tier $t lat or CPU sampling failed — kept output"
...
lane_e_rung() { # lane_e_rung <sites> [repeat-index] [is-control]
	...
	for ((s = 0; s < sites; s++)); do
		...
		for ((j = 0; j < LANE_E_SUB_CONTAINERS_PER_SITE; j++)); do
			[ "$LANE_E_PLACEMENT" != container ] || sdi=$(((s * (LANE_E_SUB_CONTAINERS_PER_SITE + LANE_E_PUB_CONTAINERS_PER_SITE) + j) % D))
		...
		for ((j = 0; j < LANE_E_PUB_CONTAINERS_PER_SITE; j++)); do
			[ "$LANE_E_PLACEMENT" != container ] || sdi=$(((s * (LANE_E_SUB_CONTAINERS_PER_SITE + LANE_E_PUB_CONTAINERS_PER_SITE) + LANE_E_SUB_CONTAINERS_PER_SITE + j) % D))
