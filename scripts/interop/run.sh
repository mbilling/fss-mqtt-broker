...
# Issue #266 (was #245's refusal): a v5 SUBSCRIBE carrying a Subscription Identifier is
# now GRANTED and delivers — this check was #245's tripwire in this lane (it expected no
# delivery, and went red the moment delivery shipped), rewritten to pin the acceptance.
# mosquitto_sub's %P renders only user properties (verified empirically — a user property
# prints, the identifier does not), so the ID-level round-trip through a foreign client is
# the Paho suite's test_subscribe_identifiers, and the byte-level property is pinned in
# crates/mqttd/tests/v5_protocol.rs; what mosquitto can honestly attest here is that the
# id-bearing SUBSCRIBE is accepted and its retained replay arrives.
mosquitto_pub -h 127.0.0.1 -p "$MQTT" -V 5 -t 'subid/kept' -m 'arrives-with-id' -r >/dev/null 2>&1 || true
got="$(mosquitto_sub -h 127.0.0.1 -p "$MQTT" -V 5 \
  -D subscribe subscription-identifier 3 -t 'subid/kept' -C 1 -W 2 2>/dev/null || true)"
expect "v5 SUBSCRIBE with a Subscription Identifier is accepted and delivers" \
  "arrives-with-id" "$got"
