...
#
# Five of the suite's 27 tests do not pass, for three different reasons, and every one is
...
EXPECTED = {
    "test_session_expiry": (
        "REAL DEVIATION (filed): a DISCONNECT's Session Expiry Interval must override "
        "the CONNECT's [MQTT-3.14.2.2.2]. conn.rs's DISCONNECT arm reads only `d.reason` "
        "and drops the properties, so a client that extends its expiry on the way out "
        "loses the session anyway. Delete this entry when the override lands."
    ),
    "test_will_delay": (
