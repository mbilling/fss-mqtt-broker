...
echo "== rendering the chart (helm template) =="
...
echo "== rendering the operator (cargo run --bin render_sample) =="
cargo run --quiet -p mqttd-operator --bin render_sample > "$WORK/operator.json"
