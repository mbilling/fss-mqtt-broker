...
The **loadgen** drives traffic on **every node** (the node list comes from `LOADGEN_NODES`,
generated by `scale-cluster.py`, so it scales with the cluster): a clean-session QoS-1
subscriber on `demo/#` per node, a persistent QoS-1 subscriber (a durable session) on
node-2, QoS-1 publishes rotated across all nodes, a retained message and a short-lived
churn connection on a rotating node — populating publish/deliver, sessions, subscriptions,
retained, inflight, and accepts panels on **all** nodes, with every publish also forwarded
cross-node to the other nodes' subscribers.
